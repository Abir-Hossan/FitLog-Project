  ...
  return (
    <article className={`plan-card ${completed ? "opacity-70" : ""}`}>
      ...
      <div className="min-w-0 flex-1">
        <h3
          className={`font-display text-lg font-bold uppercase tracking-wide text-white ${completed ? "line-through" : ""}`}
        >
          {workout.name}
        </h3>
      ...
      <div className="flex shrink-0 items-center gap-2 sm:ml-auto">
        ...
        <button
          ...
          onClick={() =>
            savedOnly ? removeSaved(workout.id) : removeFromPlan(workout.id)
          }
        >
          <X size={18} />
        </button>
