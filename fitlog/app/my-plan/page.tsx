...
export default function MyPlanPage() {
  ...
  useEffect(() => {
    setTab(new URLSearchParams(window.location.search).get('tab') === 'saved' ? 'saved' : 'plan');
  }, []);
  ...
  return (
    <main className="container-shell py-9 lg:py-12">
      <header>
        <h1 className="font-display text-4xl font-bold uppercase leading-none sm:text-5xl">MY PLAN</h1>
      ...
      <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="inline-flex w-fit rounded-xl border border-border bg-card p-1">
          <button type="button" className={`rounded-lg px-5 py-2 text-xs font-semibold ${tab === 'plan' ? 'bg-[#242a35] text-white' : 'text-muted'}`} onClick={() => setTab('plan')}>Today&apos;s Plan</button>
          <button type="button" className={`rounded-lg px-5 py-2 text-xs font-semibold ${tab === 'saved' ? 'bg-[#242a35] text-white' : 'text-muted'}`} onClick={() => setTab('saved')}>Saved</button>
        </div>
      ...
      <section className="mt-6 space-y-4">
        {visible.length === 0 ? <EmptyState /> : visible.map((workout) => <PlanCard key={workout.id} workout={workout} savedOnly={tab === 'saved'} />)}
      </section>
    </main>
  );
}
